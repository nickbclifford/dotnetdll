...
impl<'a> TryFromCtx<'a, Endian> for Unsigned {
    type Error = scroll::Error;

    fn try_from_ctx(from: &'a [u8], ctx: Endian) -> Result<(Self, usize), Self::Error> {
        let offset = &mut 0;

        let b1 = from.gread_with::<u8>(offset, ctx)? as u32;

        Ok((
            Unsigned(if b1 >> 7 == 0 {
                b1
            } else {
                let b2 = from.gread_with::<u8>(offset, ctx)? as u32;
                if b1 >> 6 == 0b10 {
                    ((b1 & 0b111111) << 8) | b2
                } else {
                    let b3 = from.gread_with::<u8>(offset, ctx)? as u32;
                    let b4 = from.gread_with::<u8>(offset, ctx)? as u32;
...
impl<'a> TryFromCtx<'a, Endian> for Signed {
    type Error = scroll::Error;
